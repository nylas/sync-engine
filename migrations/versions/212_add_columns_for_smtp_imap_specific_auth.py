...
def upgrade():
    ...
    conn.execute(text("UPDATE genericaccount SET genericaccount.imap_username "
                      "= (SELECT account._raw_address "
                      "FROM account WHERE genericaccount.id = account.id)"))
    conn.execute(text("UPDATE genericaccount SET genericaccount.smtp_username "
                      "= (SELECT account._raw_address "
                      "FROM account WHERE genericaccount.id = account.id)"))
    conn.execute(text("UPDATE genericaccount SET "
                      "imap_password_id = password_id"))
    conn.execute(text("UPDATE genericaccount SET "
                      "smtp_password_id = password_id"))
...
def downgrade():
    ...
    conn.execute(text("set @@lock_wait_timeout = 20;"))
    conn.execute(text("SET FOREIGN_KEY_CHECKS=0;"))
    conn.execute(text("ALTER TABLE event DROP CONSTRAINT "
