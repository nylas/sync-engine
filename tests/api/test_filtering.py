...
def test_thread_received_recent_date_sort(db, test_client, default_namespace):
    date1 = datetime.datetime(2015, 1, 1, 1, 1, 1)
    date2 = datetime.datetime(2012, 1, 1, 1, 1, 1)
    date3 = datetime.datetime(2013, 1, 1, 1, 1, 1)

    thread1 = add_fake_thread(db.session, default_namespace.id)
    thread1.receivedrecentdate = date1
    thread1.recentdate = date3

    thread2 = add_fake_thread(db.session, default_namespace.id)
    thread2.receivedrecentdate = date2
    thread2.recentdate = date1

    thread3 = add_fake_thread(db.session, default_namespace.id)
    thread3.receivedrecentdate = date3
    thread3.recentdate = date2
    ...
    sorted_receivedrecentdate_threads = [thread1, thread3, thread2]
    sorted_recentdate_threads = [thread2, thread1, thread3]

    receivedrecentdate_threads = \
        json.loads(test_client.get('/n/{}/threads?sort=received_recent_date'.
                                    format(default_namespace.public_id)).data)

    recentdate_threads = \
        json.loads(test_client.get('/n/{}/threads'.
                                    format(default_namespace.public_id)).data)

    for sorted_thread, api_thread in \
            zip(sorted_receivedrecentdate_threads, receivedrecentdate_threads):
        assert sorted_thread.public_id == api_thread['id']

    for sorted_thread, api_thread in \
            zip(sorted_recentdate_threads, recentdate_threads):
        assert sorted_thread.public_id == api_thread['id']
