...
def test_draft_deletes_include_version(api_client):
    ts = int(time.time())
    cursor = get_cursor(api_client, ts)
    ...
    # Create draft.
    # Len(deltas) returned == 2 for the Message, Thread created.
    api_client.post_data('/drafts/', {'subject': 'Draft subject'})

    sync_data = api_client.get_data('/delta?cursor={}'.format(cursor))
    assert len(sync_data['deltas']) == 2

    for d in sync_data['deltas']:
        if d['object'] == 'message':
            delta = d

    assert delta
    id_ = delta['id']
    version = delta['attributes']['version']
    assert delta['attributes']['version'] is not None

    # Delete draft.
    # Both draft, thread should get deleted.
    api_client.delete('/drafts/{}'.format(id_), data={'version': version})
    ...
    assert message_delta['event'] == 'delete' and \
        thread_delta['event'] == 'delete'
    assert message_delta['id'] == id_ and message_delta['version'] == version
    assert 'version' not in thread_delta
