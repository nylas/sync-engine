...
def test_remove_snippet_quoted_text():
    m = Message()
    # check that we remove quoted text (previous messages) from snippets
    body = "I'm seeing it in google but not nylas. John On Thu, Jan 21, " \
           "2016 at 1:11 PM, Karim Hamidou (Nylas) <support@nylas.com> " \
           "wrote: ##- Please type your reply above the"

    assert m.calculate_plaintext_snippet(body) == \
            "I'm seeing it in google but not nylas. John"

    body = "All good now, it seems to have caught up after a few days open " \
           ":) On Jan 26 2016, at 2:18 pm, Nylas <support@nylas.com> wrote:"

    assert m.calculate_plaintext_snippet(body) == \
            "All good now, it seems to have caught up after a few days open :)"

    body = "Thanks you, Juan! And where I can find theme's page to " \
           "download and install? Date: Thu, 22 Jan 2016 12:10:31 +0000 " \
           "From: support@nylas.com"

    assert m.calculate_plaintext_snippet(body) == \
            "Thanks you, Juan! And where I can find theme's page " \
            "to download and install?"

    body = "Hi Michael, the client error is come from xxxxxx@gmail.com. " \
           "Thanks Regards,Adrian Date: Thu, 21 Jan 2016 00:29:47 +0000 " \
           "From: support@nylas.com"

    assert m.calculate_plaintext_snippet(body) == \
            "Hi Michael, the client error is come from xxxxxx@gmail.com. " \
            "Thanks Regards,Adrian"

    body = "Test On Sunday, 31 January 2016 20:47:44 UTC-5, Charles Weill  " \
           "wrote:Just an FYI: go/types/#example_Scope does not work as is. " \
           "Running it prints:"

    assert m.calculate_plaintext_snippet(body) == \
            "Test"

    # in case of no leading body text, fall back to a simple body[:length]
    body = "On Sunday, 10 Jan 2016 10:00:01, John Doe wrote: this is a test."

    assert m.calculate_plaintext_snippet(body) == body

    # Spanish
    body = u"Si quieres algo profesional a tendrás que ir a parallels " \
           u"Saludos El 28/01/16 a las 05:50, angel jauregui escribió: " \
           u"Buen dia."

    assert m.calculate_plaintext_snippet(body) == \
            u"Si quieres algo profesional a tendrás que ir a parallels Saludos"

    body = u"Test El lunes, 1 de febrero de 2016, 11:15:03 (UTC+1), monoBOT " \
           u"monoBOT  escribió:"
    ...
    # German
    body = "Hallo, Am Freitag, 15. Januar 2016, 13:21:46 CET schrieb " \
           "Robert Scheck:"

    assert m.calculate_plaintext_snippet(body) == "Hallo,"

    # French
    body = u"Bonjour, Le 13/01/2015 09:18, Frederic Peters a écrit :"

    assert m.calculate_plaintext_snippet(body) == u"Bonjour,"

    # Portuguese
    body = u"ligações e a transmissão de dados. Sucintamente, esta é uma " \
           u"ideia geral do mundo do 'HOBBY' Cumprimentos No dia 4 de " \
           u"Novembro de 2014 às 01:16, João de Sousa <jpsousa  fe.up.pt> "\
           u"escreveu:"

    assert m.calculate_plaintext_snippet(body) == \
            u"ligações e a transmissão de dados. Sucintamente, esta é uma " \
            u"ideia geral do mundo do 'HOBBY' Cumprimentos"

    # Dutch
    body = "Test Op zaterdag 30 januari 2016 17:36:46 UTC+1 schreef " \
           "gerald...@gmail.com: great work. why did you write"
