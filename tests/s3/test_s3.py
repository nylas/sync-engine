...
# Run this test only after adding delete-permissions on the test s3 bucket for
# the test user.
@pytest.mark.skipif(True, reason='requires s3 delete-permissions')
def test_data_deletion_across_namespaces(s3_db):
    from inbox.models import Message
    from inbox.models.roles import delete_blocks

    msg = s3_db.session.query(Message).get(1)
    namespace_id = msg.namespace_id
    s3_keys = [p.block.data_sha256 for p in msg.parts]
    ...
    for i in range(len(msg.parts)):
        ...
        duplicate_msg_block = duplicate_msg.parts[i].block
        assert duplicate_msg_block.data_sha256 == s3_keys[i]
...
# Run this test only after adding delete-permissions on the test s3 bucket for
# the test user.
# USE WITH CAUTION: Will delete data from s3.
@pytest.mark.skipif(True, reason='requires s3 delete-permissions')
def test_data_deletion_within_namespace(s3_db):
    ...
    for b in s3_db.session.query(Block).filter(
            Block.namespace_id == duplicate_msg.namespace_id).all():
        b.namespace_id = namespace_id
    duplicate_msg.namespace_id = namespace_id
    s3_db.session.commit()
    ...
    for i in range(len(msg.parts)):
        ...
        assert duplicate_msg_block.size is None
        assert duplicate_msg_block.data_sha256 is None
        assert duplicate_msg_block.data is None
