  ...
  return {
    ...
    controller: function ($scope, $element, $attrs, $transclude) {
      ...
      $scope.availableTopWidth = function () {
        return parseInt($("#to_top").css("width"), 10) - 80;
      },
      ...
      $scope.adjRecipients = function () {
        ...
        var parts = to.split(",");
        $scope.recipient_lines[$scope.recipient_lines.length - 1].push({
          "email": parts[0]
        });
        if (parts.length > 1) {
          $scope.to = parts[1];
      ...
      $scope.setToInputWidth = function () {
        if ($scope.isFullComposerViewActive) {
          ...
          $(".recipient_line").last().find(".email_token").each(function () {
            available_width -= parseInt($(this).width(), 10);
          });
    ...
    link: function ($scope, $elem, $attrs, $ctrl) {
      ...
      $scope.$watch(
        ...
        function (newValue, oldValue) {
          var to_field_height = Math.max(parseInt($elem.find("#to_top").css("height"), 10) + (newValue - oldValue) * 15,
            25);
