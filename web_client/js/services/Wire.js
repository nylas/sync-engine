...
app.factory("Wire", function ($rootScope, WIRE_SERVER_URL) {
  ...
  /**
   * @var Client.callbacksRPC : stock all the callbacks which will be fired
   * when a Return from client will appear
   */
  io.SocketNamespace.prototype.callbacksRPC = {};

  /**
     * @var Client.uid : unique identifier by client, incremented by one at each
       JSON-RPC request
     */
   ...
   * @param function(result) Called when client has respond with result of
   *        the method in arguments
    ...
    for (var j = 1; j < arguments.length; j++) {
      if (Array.isArray(arguments[j])) {
        for (var i = 0; i < arguments[j].length; i++) {
          typeof arguments[j][i] === "function" &&
            callbacks.push(arguments[j][i]);
        }
  ...
  io.SocketNamespace.prototype._onClientCallRPC = function (
    data) {
    if (data.id && this.callbacksRPC[data.id]) {
      this.callbacksRPC[data.id].timerOut && clearTimeout(
        this.callbacksRPC[data.id].timerOut);
      data.result && typeof this.callbacksRPC[data.id].success ===
        "function" && this.callbacksRPC[data.id].success(
          data.result);
      data.error && typeof this.callbacksRPC[data.id].error ===
        "function" && this.callbacksRPC[data.id].error(
          data.error);
      typeof this.callbacksRPC[data.id].finaly ===
        "function" && this.callbacksRPC[data.id].finaly(
          data);
  ...
  io.SocketNamespace.prototype.onPacket = function (packet) {
    ...
    case "disconnect":
      if (this.name === "") {
        this.socket.onDisconnect(packet.reason ||
          "booted");
      } else {
    ...
    case "json":
      ...
      if (typeof data === "object" && data.method && this.registersRPC[
        data.method]) {
  ...
  var mySocket = io.connect(WIRE_SERVER_URL, {
    ...
    "reconnection limit": 1000, // don"t exponentially grow
  ...
  mySocket.on("reconnect", function (transport, attempt) {
    console.log("reconnecting by transport [" + transport +
      "] trycount: " + attempt);
  });
