...
between the different FolderSyncMonitor green threads results in two or more
...
Database sessions for each FolderSyncMonitor are held for the duration of the
action (e.g. poll, initial sync). Sessions for ZeroRPC requests are held for
the duration of the request, if they need database access.  MailSyncMonitor
...
from .models.tables import IMAPAccount, FolderSync, FolderItem, Message, Namespace
...
def uidvalidity_callback(db_session, account_id):
    def fn(folder, select_info):
        ...
        cached_validity = account.get_uidvalidity(account_id, db_session, folder)
        selected_uidvalidity = select_info['UIDVALIDITY']
        if cached_validity and not account.uidvalidity_valid(account_id,
                db_session, selected_uidvalidity, folder,
                cached_validity.uid_validity):
            raise UIDInvalid("folder: {0}, remote uidvalidity: {1}, cached uidvalidity: {2}".format(folder, selected_uidvalidity, cached_validity.uid_validity))
        return select_info
    return fn
...
class FolderSyncMonitor(Greenlet):
    ...
    def __init__(self, account_id, folder_name, email_address, provider, shared_state):
        self.folder_name = folder_name
        self.log = get_logger(account_id, 'sync')
        self.shared_state = shared_state
        self.state = None

        self.crispin_client = new_crispin(account_id, email_address, provider)

        self.state_handlers = {
                'initial': self.initial_sync,
                'initial uidinvalid': self.resync_uids('initial'),
                'poll': self.poll,
                'poll uidinvalid': self.resync_uids('poll'),
                'finish': lambda db_session: 'finish',
                }
    ...
    def _run(self):
        with session_scope() as db_session:
            ...
            # NOTE: The parent MailSyncMonitor handler could kill us at any
            # time if it receives a shutdown command. The shutdown command is
            # equivalent to ctrl-c.
            while True:
                try:
                    self.state = foldersync.state = \
                            self.state_handlers[foldersync.state](db_session)
                except UIDInvalid:
    ...
    def resync_uids(self, previous_state):
        @retry
        def _resync_uids(self, db_session, previous_state):
            ...
            return _resync_uids(self, db_session, previous_state)
        return _resync_uids

    @retry
    def initial_sync(self, db_session):
        ...
        with self.crispin_client.pool.get() as c:
            self.crispin_client.select_folder(self.folder_name,
                    uidvalidity_callback(db_session, account_id), c)

            remote_g_metadata = None
            cached_validity = account.get_uidvalidity(account_id, db_session,
                    self.folder_name)
            if cached_validity is not None:
                # If there's no cached validity it generally means we haven't
                # previously run.
                remote_g_metadata = self._retrieve_g_metadata_cache(db_session,
                    local_uids, cached_validity, c)

            if remote_g_metadata is None:
                remote_g_metadata = self.crispin_client.g_metadata(
                        self.crispin_client.all_uids(c), c)
                set_cache(os.path.join(str(account_id), self.folder_name,
                    "remote_g_metadata"), remote_g_metadata)
                self._update_validity(db_session,
            ...
            if self.crispin_client.provider == 'Gmail' and \
                ...
                self._download_expanded_threads(db_session, remote_g_metadata,
                        remote_uids, flags, c)
            else:
                # normal IMAP servers & Gmail All Mail
                full_download = self._deduplicate_message_download(db_session,
    ...
    def _safe_download(self, db_session, uids, namespace_id, c):
        ...
        for msg in raw_messages:
            ...
            with session_scope() as thread_db_session:
                thread = Thread.from_message(thread_db_session, item.message)
                thread_db_session.add(thread)
                thread_db_session.commit()
                item.message.thread_id = thread.id
                thread_db_session.close()

        return new_folderitems

    def _download_new_messages(self, db_session, uids, c):
        new_folderitems = self._safe_download(db_session, uids, c)
    ...
    def _add_new_folderitem(self, db_session, remote_g_metadata, uids, c):
    ...
    def _update_g_metadata_cache(self, db_session, remote_g_metadata, local_uids, c):
        ...
        set_cache("_".join([self.crispin_client.email_address, self.folder_name,
            "remote_g_metadata"]), remote_g_metadata)
        ...
        for uids in chunk(updated, 5*self.crispin_client.CHUNK_SIZE):
            self._update_metadata(db_session, uids, c)
        self.log.info("Updated metadata for modified messages")

    def _highestmodseq_update(self, db_session, last_highestmodseq, c):
        ...
        if uids:
            ...
            local_uids += new
            local_uids = set(local_uids).difference(
                    self._remove_deleted_messages(db_session, local_uids, remote_uids, c))
    ...
    def _download_expanded_threads(self, db_session, remote_g_metadata, uids, flags, c):
        """ UIDs, remote_g_metadata, and flags passed in are for the _folder
            ...
            needs to do next.
        """
        assert self.crispin_client.provider == 'Gmail', \
        ...
        self.crispin_client.select_folder(
                ...
                uidvalidity_callback(db_session,
                    self.crispin_client.account_id), c)
    ...
    def _deduplicate_message_download(self, db_session, remote_g_metadata, uids, c):
        ...
        if len(folderitem_only) > 0:
            self._add_new_folderitem(db_session, remote_g_metadata,
                    folderitem_only, c)
    ...
    def poll(self, db_session):
        ...
        self.log.info("polling {0} {1}".format(
            self.crispin_client.email_address, self.folder_name))

        with self.crispin_client.pool.get() as c:
            cached_validity = account.get_uidvalidity(
                    self.crispin_client.account_id, db_session, self.folder_name)
            ...
            if status['HIGHESTMODSEQ'] > cached_validity.highestmodseq:
                self.crispin_client.select_folder(self.folder_name,
                        uidvalidity_callback(db_session,
                            self.crispin_client.account_id), c)
                self._highestmodseq_update(db_session,
                        cached_validity.highestmodseq, c)

            self.shared_state['status_callback'](
                self.crispin_client.account_id, 'poll',
                (self.folder_name, datetime.utcnow().isoformat()))
            sleep(self.shared_state['poll_frequency'])

        return 'poll'

    def _remove_deleted_messages(self, db_session, local_uids, remote_uids, c):
        """ Works as follows:
    ...
    def _update_metadata(self, db_session, uids, c):
        ...
        account.update_metadata(self.crispin_client.account_id,
                db_session, self.folder_name, uids, new_flags)
        db_session.commit()
...
class MailSyncMonitor(Greenlet):
    ...
    """
    def __init__(self, account_id, email_address, provider, status_callback,
            poll_frequency=30, heartbeat=1):
        self.inbox = Queue()
        # how often to check inbox
        self.heartbeat = heartbeat
        self.log = configure_sync_logging(account_id, email_address)
    ...
    def sync(self):
        ...
        crispin_client = new_crispin(self.account_id,
                self.email_address, self.provider)
        ...
        for folder in sync_folders:
            if saved_states.get(folder) != 'finish':
                ...
                thread = FolderSyncMonitor(self.account_id, folder,
...
class SyncService(object):
    ...
    def __init__(self):
        ...
        # (Later we will want to partition these across different machines!)
            ...
            for email_address, in db_session.query(IMAPAccount.email_address)\
